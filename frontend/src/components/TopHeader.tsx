...
import { Search, Bell, LogOut, LogIn, Zap, RefreshCw } from 'lucide-react';
...
export default function TopHeader() {
    ...
    const syncYouTubeData = async () => {
        ...
        try {
            const response = await fetch("http://127.0.0.1:8000/api/v1/youtube/sync-extended", {
                method: "POST",
                    ...
                    {session?.user && (
                        <div className="flex items-center gap-2">
                            ...
                            <button
                                ...
                                title="Quick sync + full extraction pipeline"
                                className="flex items-center gap-2 bg-indigo-600/10 hover:bg-indigo-600/20 text-indigo-400 border border-indigo-600/20 px-4 py-2 rounded-lg text-sm font-medium transition-colors disabled:opacity-50"
                            >
                                <Zap size={15} className={isSyncing ? "animate-pulse" : ""} />
                                {isSyncing ? "Starting..." : activeJobId ? "Syncing..." : "Full Sync"}
                            </button>
                        </div>
                    )}
